...
class Linformer(nn.Module):
    ...
    def __init__(self, 
        ...
        get_attn = lambda attn_channels, curr_dim_k: MHAttention(input_size, head_dim, attn_channels, curr_dim_k, 
                                                                 nhead, dropout, checkpoint_level, parameter_sharing, 
                                                                 E_proj, E_proj, full_attention, causal_enc, 
                                                                 w_o_intermediate_dim, decoder_mode=False, method=method)
        get_attn_context = lambda attn_channels, curr_dim_k: MHAttention(input_size, head_dim, attn_channels, curr_dim_k, 
                                                                         nhead, dropout, checkpoint_level, parameter_sharing, 
                                                                         E_proj, E_proj, full_attention, causal_mask, 
                                                                         w_o_intermediate_dim, decoder_mode=True, method=method)
...
class LinformerEncDec(nn.Module):
    ...
    def __init__(self, 
        ...
        self.encoder = LinformerLM(num_tokens=enc_num_tokens, 
                                   ...
                                   method=enc_method)
        self.decoder = LinformerLM(num_tokens=dec_num_tokens, 
